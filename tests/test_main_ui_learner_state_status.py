...
class MainLearnerStateUIContractTest(unittest.TestCase):
    ...
    def test_stable_popover_precedes_the_slide_stage(self):
        ...
        contents = workspace.index("_render_learner_state_contents_periodic")
        slide_scale = workspace.index('key="main_slide_scale_down"')
        self.assertLess(enhance, popover)
        self.assertLess(popover, contents)
        self.assertLess(contents, slide_scale)
        self.assertIn('key="main_learner_state_popover"', workspace)
        self.assertNotIn('key="main_learner_state_reminder_slot"', workspace)
